...
export function parseJSONPath(expr){
  return applyMiddlewares(expr, lexers)
}
export function applyMiddlewares(expr, fns, context = {}){
  var head = fns[0]
    , tails = fns.slice(1)
    , preProcessor = head[0]
  ...
  if(!tails.length){
    ...
    return postProcessor(applyMiddlewares(preResult, tails, context), context)
  }
}
