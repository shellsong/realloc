...
/***/ function(module, exports, __webpack_require__) {
	...
	function actionCreatorFactory(stateGetter, collect) {
	  return function (keyPath, fn, opts) {
	    ...
	    var compiler = new _JSONPathCompiler.Compiler(keyPath);
	    var matcher = compiler.createMatcher();
	    var depMatchers = options.deps.map(function (k) {
	      return new _JSONPathCompiler.Compiler(k).createMatcher();
	    });
	    return function () {
	      ...
	      var $ = stateGetter();
	      var depValues = depMatchers.map(function (depMatcher) {
	        return depMatcher($, payloads).map(function (r) {
	          return r.value;
	        });
	      });
	      return matcher($, payloads).map(function (result) {
	        var setter = function setter(newValue) {
	          ...
	          var cur = newCur;
	          result.pwd.slice(1).forEach(function (key) {
	            if (key !== null) {
	          ...
	          if (result.name !== null) {
	            ...
	            collect(newCur, result.pwd);
	          } else {
	            collect(newValue, result.pwd);
	          }
	        };
	        var args = payloads.concat(depValues).concat([result.value, setter]);
